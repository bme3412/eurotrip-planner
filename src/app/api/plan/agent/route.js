...
import { getTripWithDetails, swapActivity, updateActivity } from '@/lib/trips/tripState';
...
async function execGetCityAttractions({ city, interests = [], exclude_names = [] }) {
...
async function execGetPlaceDetails({ place_id }) {
  const FIELD_MASK = [
...
async function execSearchNearby({ latitude, longitude, type, radius = 500 }) {
  const FIELD_MASK = [
...
async function execUpdateItinerary({ trip_id, activity_id, new_activity, reason }, emit) {
  try {
    const replacement = await swapActivity(activity_id, new_activity, reason);

    // Fetch the updated trip to find the day/time context for the UI event
...
function buildSystemPrompt(trip, cityData) {
...
function buildToolSummary(name, args, result) {
  switch (name) {
    case 'get_city_attractions':
