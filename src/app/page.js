            ...
            <div className="bg-white rounded-3xl p-6 md:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-blue-50 relative z-10">
              <div className="mb-8 text-center lg:text-left">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-1">
                  {dates?.start && dates?.end ? "Your Travel Dates" : "When are you going?"}
                </h3>
                <p className="text-gray-500 text-base font-medium">
                  {dates?.start && dates?.end
                    ? `${Math.ceil((new Date(dates.end) - new Date(dates.start)) / 86400000)} nights — we'll rank all 220 cities for exactly these days`
                    : "We'll rank all 220 cities for exactly when you're going"}
                </p>
              </div>
              ...
              {dates?.start && dates?.end && (
                <div className="mt-6 flex justify-center lg:justify-start">
                  <button
                    onClick={submit}
                    disabled={loading}
                    className="group flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-full hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {loading ? (
                      <>
                      ...
                      <>
                        Rank all 220 cities for these dates
