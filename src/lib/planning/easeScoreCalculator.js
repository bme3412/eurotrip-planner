...
/**
 * Get connection details from one city to another
 * Used to calculate how well a candidate city connects to the end city
 *
 * @param {string} fromCityId - The city to travel FROM
 * @param {string} toCityId - The city to travel TO
 * @returns {Promise<{travelMinutes: number, transportType: string, transportTime: string} | null>}
 */
export async function getConnectionToCity(fromCityId, toCityId) {
  if (!fromCityId || !toCityId) return null;
  if (fromCityId === toCityId) return { travelMinutes: 0, transportType: 'same', transportTime: '0m' };

  const fromSlug = fromCityId.toLowerCase().replace(/\s+/g, '-');
  const toSlug = toCityId.toLowerCase().replace(/\s+/g, '-');

  try {
    const data = await loadConnections(fromSlug);
    const destinations = data.destinations || [];

    // Find the destination city in connections
    const connection = destinations.find(dest => {
      const destSlug = dest.city.toLowerCase().replace(/\s+/g, '-');
      return destSlug === toSlug;
    });
    ...
    return {
      ...
      transportType: best.type,
      transportTime: timeStr,
    };
  } catch (err) {
    console.warn(`[easeScoreCalculator] Failed to get connection ${fromCityId} → ${toCityId}:`, err);
    return null;
  }
}

/**
 * Batch get connections from multiple candidate cities to an end city
 * More efficient than calling getConnectionToCity multiple times
 *
 * @param {string[]} candidateCityIds - Array of candidate city IDs
 * @param {string} endCityId - The end city to check connections to
 * @returns {Promise<Map<string, {travelMinutes: number, transportType: string}>>}
 */
export async function batchGetConnectionsToCity(candidateCityIds, endCityId) {
  const results = new Map();

  if (!endCityId || !candidateCityIds?.length) return results;

  // Process in batches of 5 to avoid overwhelming the server
  const BATCH_SIZE = 5;
  for (let i = 0; i < candidateCityIds.length; i += BATCH_SIZE) {
    const batch = candidateCityIds.slice(i, i + BATCH_SIZE);
    const promises = batch.map(async (cityId) => {
      const connection = await getConnectionToCity(cityId, endCityId);
      return { cityId, connection };
    });

    const batchResults = await Promise.all(promises);
    for (const { cityId, connection } of batchResults) {
      if (connection) {
        results.set(cityId, connection);
      }
    }
  }
