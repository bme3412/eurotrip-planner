 ...
 * thresholds (73 / 64 / 55, see scoringConfig.json) and the labels used by TierBadge.
 *
...
export function scoreToBand(finalScore) {
  ...
  if (s >= 73) return QUALITATIVE_BANDS[1];
  if (s >= 64) return QUALITATIVE_BANDS[2];
  if (s >= 55) return QUALITATIVE_BANDS[3];
