...
// 8 most popular European cities with pre-computed blur placeholders
...
const PopularCitiesSection = ({ cities = [] }) => {
  ...
  // Staggered loading: reveal cards in batches when section becomes visible
  const handleSectionVisible = useCallback(() => {
    setSectionVisible(true);

    // First batch: cards 0-3 (immediate)
    setVisibleCards(new Set([0, 1, 2, 3]));

    // Second batch: cards 4-7 (after delay)
